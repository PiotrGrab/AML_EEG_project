...
(mean validation accuracy 0.793 across subjects 1–3)
...
**Headline.** The EEG Conformer reaches a mean cross-subject accuracy of ~71 %
on a 50 % chance baseline, but the headline number hides a very wide spread:
S3 reaches 91 %, while S2 sits essentially at chance (~54 %). This kind of
...
decodable motor imagery patterns. S2 and S5 are plausibly such cases; S4 is
...
training curves with no obvious instability across all five subjects, and
the strong subjects (S1, S3) achieve a per-class balance close to symmetric,
showing the architecture is capable of unbiased decoding when the underlying
signal is clean enough.

**Per-class asymmetries.** At the individual level the picture is decidedly
not tidy. S2 strongly favours left-hand (72 % vs 35 %), S5 even more so
(83 % vs 33 %), while S4 inverts that pattern (58 % vs 75 %). These biases
do not cancel out at the population level — averaging across subjects gives
left-hand accuracy 75.8 % vs right-hand 66.1 %, a ~10 percentage-point bias
toward predicting *left* on the aggregate confusion matrix. A deployed BCI
for any of the weak subjects would feel very different in practice from the
mean number on the tin.
...
the model spreads its attention more broadly: averaged across subjects, the
**frontal** region shows the highest mean saliency (~0.48), with **parietal**
(~0.45) close behind, and **motor cortex** (~0.39) and **centro-parietal**
(~0.33) trailing. The per-class saliency profiles for left- vs right-hand
imagery are also nearly identical, meaning the Conformer is not discriminating
the two classes by looking at *different* electrodes for each class — it uses
the same spatial pattern but exploits temporal/spectral differences inside the
convolutional front-end. This is consistent with how the model is built (the
spatial conv runs once across all channels) but does diverge from the
classical neurophysiological story of contralateral mu suppression.
